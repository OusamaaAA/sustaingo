  ...
  Future<void> _submitReservation() async {
    ...
    try {
      ...
      final csrfResponse = await http.get(
        Uri.parse('https://sustaingobackend.onrender.com/api/csrf/'),
      ...
      // Update the endpoint to include bagId
      final url = Uri.parse(
      ...
      if (response.statusCode == 201) {
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder:
                (_) => ReservationConfirmationScreen(
                  bagContents: [],
                ), // Add actual contents if available
          ),
        );
      } else {
        ...
        if (response.body.trim().startsWith('<!DOCTYPE') ||
          ...
          try {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Reservation failed: ${error['detail'] ?? response.body}',
                ),
              ),
            );
          } catch (e) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Reservation failed: ${response.body}')),
            );
          }
        }
      }
